...
# 启用模拟登录（开发和生产环境都支持，无需后端）
# 设置为 true：强制使用模拟登录
# 设置为 false：强制使用真实API
# 不设置：开发模式默认使用模拟，生产模式默认使用真实API（失败时自动降级到模拟）
VITE_USE_MOCK=true
```

## 测试账号

**模拟登录模式下可使用以下测试账号（开发和生产环境都支持）：**
...
**模拟模式说明：**
- 开发模式：默认启用模拟登录，无需后端即可运行
- 生产模式：如果真实API调用失败（404等），会自动降级到模拟登录
- 强制模式：设置 `VITE_USE_MOCK=true` 可强制使用模拟登录（开发和生产都生效）
